...
(Coffee Lake 2.6Ghz base, 4.5Ghz max) running Fedora 37 with a 6.0.18 kernel.
Since the laptop has 12 hyperthreaded cores, we use that as the max reader
concurrency in all the tests below.


## `btrfs-ublk` results

Here is a quick, but representative benchmark of `btrfs-ublk` serving data
generated by a `memset` in a modified `tgt_loop.cpp` server from `ubdsrv`. 
The `fio` settings are intended to be apples-to-apples with all the other
"laptop" comparisons below.  Performance under sustained load would be
proportionally lower due to thermal throttling.
...
TODO / WIP: Re-run `benchmark_matrix.py` with FC37 (faster than FC36, huh),
and summarize the results.  Touch on the page cache effects of `mmap`.

TODO: Add a `ublk` mode where virtual reads are backed by `O_DIRECT` reads
from local SSD to see how much worse things get.


### What's the bottleneck for `btrfs-ublk`?
...
physical cores (2Ghz regular, 3.7Ghz turbo), and 256GiB of DDR4.
